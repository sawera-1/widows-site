...
@section('title', 'My Orders — Company Name')
                ...
                @foreach($orders as $order)
                    <div class="order-card">
                        <div class="oc-meta">
                            ...
                            <div class="oc-date">{{ $order->created_at->format('M j, Y - g:i A') }}</div>
                            <div class="oc-status {{ $order->payment_status === 'paid' ? 'paid' : '' }}">{{ $order->status }} &middot; {{ $order->payment_status }}</div>
                        </div>
                        <div style="text-align: right; display: flex; flex-direction: column; gap: 12px; align-items: flex-end;">
                            <div class="oc-total">&pound;{{ number_format($order->total, 2) }}</div>
                            <a href="{{ route('account.orders.show', $order->id) }}" class="btn-view">View Details</a>
                        </div>
                    </div>
                @endforeach
            </div>
