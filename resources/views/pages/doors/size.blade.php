...
@section('title', 'Configure Your uPVC Entrance Door — uPVC Doors')
@section('meta_description', 'Customize dimensions, handing, colors, glazing, and locking hardware for your uPVC entrance door with instant pricing.')
...
<div class="wdc-container">
    ...
    <div class="cfg-header">
        <h1>Door {{ $currentDoor['name'] }} — Georgian bar</h1>
        <div class="meta" id="headerMeta">with obscure glazing • {{ $currentW }}mm × {{ $currentH }}mm</div>
        <div class="thermal-free-badge"> Thermal upgrade FREE (A-Rated Insulation Core)</div>
    </div>

    {{-- 3-Column Layout: LEFT 32% | CENTER 38% | RIGHT 30% --}}
    <div class="three-col">
        ...
        <div class="cfg-col">
                ...
                <div class="cfg-section-body">
                    <div class="dim-grid">
                        <div class="size-field">
                            ...
                            <div class="inp-wrap">
                                <input type="number" id="cfgWidth" value="{{ $currentW }}" min="700" max="1150" step="1">
                        ...
                        <div class="size-field">
                            ...
                            <div class="inp-wrap">
                                <input type="number" id="cfgHeight" value="{{ $currentH }}" min="1800" max="2400" step="1">
                                <span class="unit">mm</span>
                            </div>
                ...
                <div class="cfg-section-body">
                    <div class="handle-grid" id="handleGrid">
                        @foreach($handleOptions as $h)
                            <button type="button" class="handle-card {{ $loop->first ? 'is-selected' : '' }}" data-key="{{ $h['key'] }}" data-price="{{ $h['price'] }}">
                    ...
                    <div class="field-group">
                        ...
                        <div class="swatch-grid" id="glassGrid">
                            @foreach($glassOptions as $g)
                                <button type="button" class="swatch-btn {{ $loop->first ? 'is-selected' : '' }}" data-key="{{ $g['key'] }}" data-price="{{ $g['price'] }}">
                ...
                <div class="cfg-section-body">
                    <div class="field-group">
                        ...
                        <select class="wdc-select" id="sel-cill">
                            @foreach($cillOptions as $k => $cill)
                                <option value="{{ $k }}" data-price="{{ $cill['price'] }}">{{ $cill['name'] }} {{ $cill['price'] ? '(+£'.$cill['price'].')' : '' }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="field-group">
            ...
            {{-- 7. Locking Cylinder & Hardware Options --}}
                ...
                <div class="cfg-section-body">
                    ...
                    <div class="field-group">
                        ...
                        <select class="wdc-select" id="sel-letterbox">
                            @foreach($letterboxOptions as $k => $lb)
                                <option value="{{ $lb['name'] }}" data-price="{{ $lb['price'] }}">{{ $lb['name'] }} {{ $lb['price'] ? '(+£'.$lb['price'].')' : '' }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="field-group">
                        ...
                        <select class="wdc-select" id="sel-knocker">
                            @foreach($knockerOptions as $k => $kn)
                                <option value="{{ $kn['name'] }}" data-price="{{ $kn['price'] }}">{{ $kn['name'] }} {{ $kn['price'] ? '(+£'.$kn['price'].')' : '' }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="field-group">
        ...
        {{-- COLUMN 2 (CENTER 38%) — Live Interactive Door Preview --}}
        <div class="preview-col">
            <div class="pv-header">
                <div class="pv-title" id="pvTitle">Door {{ $currentDoor['name'] }}</div>
                <div class="pv-dims" id="pvDims">{{ $currentW }}mm (W) × {{ $currentH }}mm (H)</div>
            </div>

            <div class="door-container-outer" id="doorPreviewBox">
                @if(($currentDoor['type'] ?? 'single') === 'flush')
                    @include('pages.doors.flushdoor-graphic', ['design' => $designKey])
                @elseif(($currentDoor['type'] ?? 'single') === 'stable')
                    @include('pages.doors.stabledoor-graphic', ['design' => $designKey])
                @else
                    @include('pages.doors.framedoor', ['design' => $designKey])
                @endif
            </div>
        </div>

        {{-- COLUMN 3 (RIGHT 30%) — Smart Info & Summary Table --}}
        <div class="info-col">
            <div class="smart-info-panel">
                ...
                <table class="summary-table">
                    <tr><td>Door Model:</td><td id="s-model">Door {{ $currentDoor['name'] }} - Georgian bar</td></tr>
                ...
                <div class="price-total-box">
                    ...
                    <div class="pt-value" id="totalPriceDisplay">£{{ number_format($currentDoor['basePrice'], 2) }}</div>
                ...
                <form action="/cart/add" method="POST" id="atcForm">
                    @csrf
                    <input type="hidden" name="product_name" value="Door {{ $currentDoor['name'] }}">
                    <input type="hidden" name="design" value="{{ $designKey }}">
                    <input type="hidden" name="product_type" value="door">
                    <input type="hidden" name="width" id="hiddenW" value="{{ $currentW }}">
                    <input type="hidden" name="height" id="hiddenH" value="{{ $currentH }}">
                    <input type="hidden" name="panes_json" id="hiddenPanesJson">
                    <input type="hidden" name="price" id="hiddenPrice" value="{{ $currentDoor['basePrice'] }}">
                    <input type="hidden" name="line_total" id="hiddenLineTotal" value="{{ $currentDoor['basePrice'] }}">
...
<script>
(function(){
    const BASE_PRICE = {{ $currentDoor['basePrice'] }};
    let colorPrice = 0;
    let glassPrice = 0;
    let handlePrice = 0;
    let cillPrice = 0;
