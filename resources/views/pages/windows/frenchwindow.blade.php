...
@section('title', 'Choose Your French Window Style — uPVC Windows')
@section('meta_description', 'Browse 7 French window frame designs across Standard French and Flush French styles. Select a design to price and order online.')
...
<main id="content">
    <section class="w-full bg-white dark:bg-black wdc-section" aria-labelledby="wdc-heading">
        <div class="wdc-container">
            ...
            {{-- STANDARD FRENCH WINDOWS SECTION (7 CASES) --}}
            <div class="french-section-title">
                <span>Standard French windows</span>
            </div>

            <div class="wdc-grid" role="list">
                @foreach($frenchCases as $case)
                    <a href="/windows/size?design={{ $case['slug'] }}"
                       class="wdc-card"
                       role="listitem"
                       aria-label="Select {{ $case['name'] }} Standard French window">
                        ...
                        <div class="wdc-window-wrap">
                            ...
                            <div class="upvc-window {{ $isWide ? 'is-wide' : '' }}"
                                 ...
                                 role="img"
                                 aria-label="{{ $case['name'] }} frame preview">
                                @foreach($case['cells'] as $cell)
                                    <div class="upvc-pane {{ !empty($cell['o']) ? 'is-open' : '' }}"
                        ...
                        <div class="wdc-card-body">
                            <span class="wdc-card-name">{{ $case['name'] }}</span>
                            <span class="text-xs text-gray-400 dark:text-gray-600 leading-relaxed">{{ $case['desc'] }}</span>
