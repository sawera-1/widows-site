...
<div class="wdc-container">
    ...
    <div class="three-col">
                    ...
                    <div class="field-group">
                        ...
                        <div class="swatch-grid" id="glassGrid">
                            @foreach($glassOptions as $g)
                                <button type="button"
                                    class="swatch-btn {{ $loop->first ? 'is-selected' : '' }}"
                                    data-field="glass" data-key="{{ $g['key'] }}">
                ...
                <div class="cfg-section-body">
                    ...
                    <div class="handle-grid" id="handleGrid">
                        @foreach($handleOptions as $h)
                            <button type="button"
                                class="handle-card {{ $loop->first ? 'is-selected' : '' }}"
                                data-field="handle" data-key="{{ $h['key'] }}">
                                <div class="handle-illus">
                                    @if($h['key'] === 'monkey-tail')
                                        {{-- Monkey-tail: curved bar handle --}}
                                    ...
                                    @else
                                        {{-- Espag lever handles (white / chrome / gold / satin / black) --}}
                                        <div class="h-espag {{ $h['css'] }}">
        ...
        {{-- ══════════════════════════════════════════════════════
             COLUMN 2 — Live window preview
        ══════════════════════════════════════════════════════ --}}
        <div class="preview-col">
            <div class="pv-label" id="pvLabel">{{ $cfg['name'] }}</div>
            ...
            <div class="pv-pane-indicator" id="pvPaneIndicator" style="{{ $paneCount <= 1 ? 'display:none;' : '' }}">
                Pane <span id="pvPaneNum">1</span> selected
            </div>
        </div>

        {{-- ══════════════════════════════════════════════════════
             COLUMN 3 — Guidance panel
        ══════════════════════════════════════════════════════ --}}
        <div class="guidance-col" id="guidanceCol">
            ...
            {{-- Opener guidance (shown when opener section is active) --}}
...
(function(){
    ...
    const HINGE_LABELS = @json($hingeTypes);
    const VENT_LABELS  = @json($trickleVents);
    const BLIND_LABELS = @json($blindOptions);
    const BAR_LABELS   = @json($barOptions);
    const DECO_LABELS  = @json($decoOptions);
