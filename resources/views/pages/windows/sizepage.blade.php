...
@section('title', 'Configure Your Window — uPVC Windows')
...
<div class="wdc-container">
    ...
    <div class="cfg-header">
        <h1>{{ $cfg['name'] }}</h1>
        <div class="meta" id="headerMeta">{{ $selected['width'] }}mm × {{ $selected['height'] }}mm</div>
    </div>

    {{-- 3-Column Layout: LEFT 30% | CENTER 40% | RIGHT 30% --}}
    <div class="three-col">

        {{-- ══════════════════════════════════════════════════════
             COLUMN 1 (LEFT 30%) — Product Options Control Panel
        ══════════════════════════════════════════════════════ --}}
        <div class="cfg-col">

            {{-- ── 1. Size & Pane Selector ── --}}
                ...
                <div class="cfg-section-body">
                    <div class="dim-grid">
                        <div class="size-field" id="wrapWidth">
                            ...
                            <div class="inp-wrap">
                                <input type="number" id="cfgWidth" value="{{ $selected['width'] }}" min="300" max="2400" step="1" placeholder="1200">
                        ...
                        <div class="size-field" id="wrapHeight">
                            ...
                            <div class="inp-wrap">
                                <input type="number" id="cfgHeight" value="{{ $selected['height'] }}" min="300" max="2400" step="1" placeholder="1500">
                                <span class="unit">mm</span>
                            </div>
                            <p class="hint">300 – 2400 mm</p>
                        </div>
                    ...
                    <div style="margin-top:14px; padding-top:12px; border-top:1px dashed #e5e5e5;">
                        ...
                        <div class="pane-tabs" id="paneTabs">
                            @for($i = 0; $i < $paneCount; $i++)
                                <button type="button" class="pane-tab {{ $i === 0 ? 'is-active' : '' }}" data-pane="{{ $i }}">
                                    Pane {{ $i + 1 }}
                                </button>
                            @endfor
                        </div>
            ...
            <div class="cfg-section is-open" id="sec-frame-colour" data-info="info-frame-colour">
                ...
                <div class="cfg-section-body">
                    <div class="color-swatch-grid" id="frameColorGrid">
                        @foreach($frameColours as $fc)
                            <button type="button" class="color-btn {{ $loop->first ? 'is-selected' : '' }}" data-color-key="{{ $fc['key'] }}" data-color-hex="{{ $fc['color'] }}" data-border-hex="{{ $fc['border'] }}" data-price="{{ $fc['price'] }}">
                                <span class="c-dot" style="background:{{ $fc['color'] }};"></span>
                                <span class="c-name">{{ $fc['name'] }}</span>
                                <span class="c-price">{{ $fc['price'] ? '+£'.$fc['price'] : 'Included' }}</span>
                            </button>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="cfg-section-body">
                    ...
                    <div class="field-group">
                        ...
                        <select class="wdc-select" id="sel-trickle-vent" data-info-trigger="info-vent">
                            @foreach($trickleVents as $k => $label)
                                <option value="{{ $k }}">{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="cfg-section-body">
                    <div class="field-group">
                        ...
                        <select class="wdc-select" id="sel-glazing-spec" data-info-trigger="info-glass">
                            @foreach($glazingSpecs as $k => $label)
                                <option value="{{ $k }}">{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="field-group">
                        ...
                        <select class="wdc-select" id="sel-glazing-safety" data-info-trigger="info-safety">
                            @foreach($glazingSafety as $k => $label)
                                <option value="{{ $k }}">{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="field-group">
                        ...
                        <div class="swatch-grid" id="glassGrid">
                            @foreach($glassOptions as $g)
                                <button type="button" class="swatch-btn {{ $loop->first ? 'is-selected' : '' }}" data-key="{{ $g['key'] }}" data-info-trigger="info-glass">
                                    <span class="swatch glass-swatch glass-{{ $g['key'] }}"></span>
                                    <span class="sname">{{ $g['name'] }}</span>
                                    <span class="sprice">{{ $g['price'] ? '+£'.$g['price'] : 'Included' }}</span>
                                </button>
                            @endforeach
                        </div>
                    </div>
            ...
            <div class="cfg-section is-open" id="sec-handle" data-info="info-handle">
                ...
                <div class="cfg-section-body">
                    <div class="handle-grid" id="handleGrid">
                        @foreach($handleOptions as $h)
                            <button type="button" class="handle-card {{ $loop->first ? 'is-selected' : '' }}" data-key="{{ $h['key'] }}" data-handle-color="{{ $h['color'] }}" data-info-trigger="info-handle">
                                <div class="handle-illus">
                                    @if($h['key'] === 'monkey-tail')
                                        <div class="h-monkey-tail"><div class="mt-shaft"></div><div class="mt-tail"></div></div>
                                    @elseif($h['key'] === 'tear-drop')
                                        <div class="h-tear-drop"><div class="td-shaft"></div><div class="td-drop"></div></div>
                                    @else
                                        <div class="h-espag h-{{ $h['key'] }}"><div class="h-backplate"></div><div class="h-lever"></div><div class="h-boss"></div></div>
                                    @endif
                                </div>
                                <div class="hname">{{ $h['name'] }}</div>
                                <div class="hprice">{{ $h['price'] ? '+£'.$h['price'] : 'Included' }}</div>
                            </button>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="cfg-section-body">
                    <div class="field-group">
                        ...
                        <select class="wdc-select" id="sel-blind" data-info-trigger="info-blind">
                            @foreach($blindOptions as $k => $label)
                                <option value="{{ $k }}">{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="field-group" id="wrap-blind-color">
                        ...
                        <select class="wdc-select" id="sel-blind-color" data-info-trigger="info-blind">
                            @foreach($blindColors as $k => $label)
                                <option value="{{ $k }}">{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="cfg-section-body">
                    <div class="field-group">
                        ...
                        <select class="wdc-select" id="sel-cill" data-info-trigger="info-cill">
                            @foreach($cillOptions as $k => $label)
                                <option value="{{ $k }}">{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="cfg-section-body">
                    <div class="field-group">
                        ...
                        <select class="wdc-select" id="sel-locks" data-info-trigger="info-locks">
                            @foreach($lockOptions as $k => $label)
                                <option value="{{ $k }}">{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="cfg-section-body">
                    <div class="field-group">
                        ...
                        <select class="wdc-select" id="sel-restrictor" data-info-trigger="info-restrictors">
                            @foreach($restrictorOptions as $k => $label)
                                <option value="{{ $k }}">{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="cfg-section-body">
                    <div class="field-group">
                        ...
                        <select class="wdc-select" id="sel-fixing-kit" data-info-trigger="info-fixing">
                            @foreach($fixingKitOptions as $k => $label)
                                <option value="{{ $k }}">{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="field-group">
                        ...
                        <select class="wdc-select" id="sel-extender" data-info-trigger="info-extenders">
                            @foreach($extenderOptions as $k => $label)
                                <option value="{{ $k }}">{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="cfg-section-body">
                    ...
                    <div class="field-group">
                        ...
                        <select class="wdc-select" id="sel-deco" data-info-trigger="info-bars">
                            @foreach($decoOptions as $k => $label)
                                <option value="{{ $k }}">{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
        ...
        <div class="preview-col">
            <div class="pv-header">
                <div class="pv-title" id="pvTitle">{{ $cfg['name'] }}</div>
                <div class="pv-dims" id="pvDims">{{ $selected['width'] }}mm × {{ $selected['height'] }}mm</div>
            </div>

            <div class="window-container-outer">
                {{-- Live CSS Window Component --}}
                <div class="upvc-window" id="mainWindow" role="img" aria-label="Window configuration live visual preview">
                    {{-- Rendered dynamically by JS paintMainWindow() --}}
                </div>
                {{-- External Cill bar graphic --}}
        ...
        <div class="info-col">
            ...
            <div class="smart-info-panel">
                ...
                {{-- 9. Internal Magnetic Blinds Info --}}
                ...
                {{-- 13. Installation Fixing Kit Info --}}
                ...
                {{-- 15. Glazing Bars & Decoration Info --}}
...
<script>
(function(){
    const WINDOW_FRAMES   = @json($windowFrames);
    const FRAME_COLOURS   = @json($frameColours);
    const GLASS_OPTIONS   = @json($glassOptions);
    const HANDLE_OPTIONS  = @json($handleOptions);
    const BASE_PRICES     = @json($basePrices);
    const SELECTED        = @json($selected);
    const SASH_STYLE      = @json(request('sash_style', 'equal'));
    const GLAZING_DESIGN   = @json(request('glazing_design', 'non-georgian'));
