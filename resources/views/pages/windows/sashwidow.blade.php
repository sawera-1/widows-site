...
@section('title', 'Choose Your Sliding Sash Window Style — uPVC Windows')
@section('meta_description', 'Select your sliding sash style and glazing bar design to get an instant online quote.')
...
<main id="content">
    <section class="w-full bg-white dark:bg-black wdc-section" aria-labelledby="wdc-heading">
        <div class="wdc-container">
            ...
            @if(!$selectedSash)
                {{-- ══════════════════════════════════════════════════════
                     LEVEL 1: CHOOSE SASH STYLE (5 Options)
                ══════════════════════════════════════════════════════ --}}
                ...
                <div class="wdc-grid" role="list">
                    @foreach($sashStyles as $key => $style)
                        <a href="/windows/sash-windows?sash_style={{ $key }}"
                           class="wdc-card"
                           role="listitem"
                           aria-label="Select {{ $style['name'] }} sliding sash style">
                            ...
                            <div class="wdc-window-wrap">
                                ...
                                @endphp
                                <div class="upvc-sash-window {{ $isSwept ? 'is-swept' : '' }} {{ $isArch ? 'is-arch' : '' }}"
                                     style="--sash-rows:{{ $rowsCss }};"
                                     role="img"
                            ...
                            <div class="wdc-card-body">
                                <span class="wdc-card-name">{{ $style['name'] }}</span>
                                <span class="wdc-card-desc">{{ $style['desc'] }}</span>
                                
                                @if(!empty($style['warning']))
            ...
            @else
                {{-- ══════════════════════════════════════════════════════
                     LEVEL 2: CHOOSE GLAZING-BAR DESIGN (10 Options)
                ══════════════════════════════════════════════════════ --}}
                <div class="selected-style-bar">
                    <span>Selected Sash Style: <span class="text-black dark:text-white font-extrabold">{{ $selectedSash['name'] }}</span></span>
                ...
                <div class="text-center mb-10 md:mb-14">
                    <h1 id="wdc-heading" class="text-3xl md:text-4xl font-extrabold tracking-tight text-black dark:text-white mb-3" style="letter-spacing:-0.02em;">
                        Choose Glazing-Bar Design for {{ $selectedSash['name'] }}
                    </h1>
                    <p class="text-base text-gray-500 dark:text-gray-400 max-w-xl mx-auto leading-relaxed">
                ...
                <div class="wdc-grid-10" role="list">
                    @foreach($glazingDesigns as $designKey => $design)
                        <a href="/windows/size?design=sash&sash_style={{ $selectedSashKey }}&glazing_design={{ $designKey }}"
                           class="wdc-card"
                           role="listitem"
                           aria-label="Select {{ $design['name'] }} glazing bar design">
                            ...
                            <div class="wdc-window-wrap">
                                ...
                                @endphp
                                <div class="upvc-sash-window {{ $isSwept ? 'is-swept' : '' }} {{ $isArch ? 'is-arch' : '' }} {{ $barClass }}"
                                     style="--sash-rows:{{ $rowsCss }};"
                                     role="img"
                                     aria-label="{{ $design['name'] }} preview">
                                    <div class="upvc-pane pane-top"></div>
                                    <div class="upvc-pane pane-bottom"></div>
                                </div>
                            </div>
                            ...
                            <div class="wdc-card-body">
                                <span class="wdc-card-name">{{ $design['name'] }}</span>
                                <span class="wdc-card-desc">{{ $design['desc'] }}</span>
