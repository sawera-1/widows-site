...
<div class="admin-wrapper">
    ...
    <aside class="admin-sidebar" id="adminSidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="{{ route('admin.enquiries.index') }}" class="nav-item {{ request()->routeIs('admin.enquiries.*') ? 'active' : '' }}">
                ...
                @if($unreadEnquiries > 0)
                    <span style="background:#fff; color:#111; border-radius:999px; padding:2px 8px; font-size:0.6875rem; font-weight:700; margin-left:auto;">{{ $unreadEnquiries }}</span>
                @endif
            </a>

            <div class="nav-section">Operations</div>
            <a href="{{ route('admin.delivery.index') }}" class="nav-item {{ request()->routeIs('admin.delivery.*') ? 'active' : '' }}">
        ...
        <a href="{{ route('admin.settings.index') }}" style="display:flex; align-items:center; gap:10px; padding:16px 24px; border-top:1px solid #1E1E1E; flex-shrink:0; text-decoration:none; transition:background 0.15s;" onmouseover="this.style.background='#1a1a1a'" onmouseout="this.style.background='transparent'">
            ...
            <div style="min-width:0; flex:1;">
                <div style="font-size:0.8125rem; font-weight:600; color:#fff; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{{ Auth::user()->name }}</div>
                <div style="font-size:0.6875rem; color:#666;">{{ Auth::user()->role ?? 'Administrator' }}</div>
            </div>
        </a>
    </aside>
    ...
    <main class="admin-main">
        <header class="admin-header">
            <div class="header-left">
                ...
                <span style="font-size:0.8125rem; color:#6B6B6B;">Welcome back, <strong style="color:#111;">{{ Auth::user()->name }}</strong></span>
            </div>
            <div class="header-right">
                <a href="{{ route('home') }}" target="_blank" class="btn btn-outline" style="min-width:120px; height:40px; justify-content:center;">
                ...
                <form method="POST" action="{{ route('admin.logout') }}" style="display:inline;" onsubmit="return confirm('Are you sure you want to log out?');">
                    @csrf
